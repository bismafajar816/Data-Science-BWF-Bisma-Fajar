...
def quick_sort(data, attribute):
    ...
    left = [x for x in data if x[attribute] < pivot[attribute]]
    middle = [x for x in data if x[attribute] == pivot[attribute]]
    right = [x for x in data if x[attribute] > pivot[attribute]]
...
def search_by_attribute(data, attribute, value):
    result = []
    for item in data:
        if str(value).lower() in str(item[attribute]).lower():
            result.append(item)
    return result
...
from searching_algorithms import search_by_attribute
...
def display_cars_data(data):
    ...
    table = [list(car.values()) for car in data]
...
def search_data():
    ...
    attribute = attr_dict.get(attr_choice, "Name")
    query = input(f"\nEnter search query for {attribute}: ")

    results = search_by_attribute(cars_data, attribute, query)
    if results:
        print(f"\nSearch results for '{query}' in {attribute}:")
        display_cars_data(results)
    else:
        print(f"\nNo results found for '{query}' in {attribute}")
